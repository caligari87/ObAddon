...
function Level_determine_map_size(LEV)
  ...
  if ob_size == "mixed" then
    local MIXED_PROBS =
    {
      ...
      small=30, regular=80, large=20, extreme=5
    }
  ...
  if ob_size == "prog" or ob_size == "epi" then
    ...
    --local SIZES = { small=20, regular=34, large=48, extreme=72 }
     local SIZES = { small=26, regular=36, large=52, extreme=72, stretched=72 }

    W = SIZES[ob_size]
  end

  if not W and ob_size != "stretched" then
...
function Level_choose_darkness()
  ...
  if rand.odds(prob) then
    ...
    LEVEL.is_dark = true
    LEVEL.sky_light  = rand.pick({ 104,112,120,128,136 }) --120
    LEVEL.sky_shadow = rand.pick({ 24,32,40 }) --24
  end
end
